...
class EnvironmentConfig:
    def __init__(self):
        # Environment paths
        self.paths = {
            'local': "/Users/xiang/Desktop/dapi_exchange/",
            'test': "/share/infa_shared/TgtFiles/bi/liscron/",
            'pro': "/share/infa_shared/TgtFiles/bi/liscron/"
        }

        # Minio URLs
        self.minio_urls = {
            'local': "cloud-minio-data-10-5-3-176:8082",
            'test': "10.5.3.178:9000",
            'pro': "10.5.1.31:9000"  # Add production URL if needed
        }

        # API URLs
        self.api_urls = {
            'local': "http://10.5.3.176:8082",
            'test': "http://10.5.3.176:8082",
            'pro': "http://dapi.klhic.com:4430"
        }

        # Minio credentials
        self.minio_credentials = {
            'local': ("igiylPwL3gz8CLrT0M1a", "pLNbAHRj1nT0pz3Bsj9M3In15GbXnMDqblUbFTuS"),
            'test': ("igiylPwL3gz8CLrT0M1a", "pLNbAHRj1nT0pz3Bsj9M3In15GbXnMDqblUbFTuS"),
            'pro': ("IjEA7mUWjM1wPbMk16Lz", "8XE3KnLBfPFcIbrLnE06xZ6L3hEUIICrm51n2krk")  # Add production credentials if needed
        }

        # Login passwords
        self.login_passwords = {
            'local': "a69292766d57ab36b4e072ef1f0a1248",
            'test': "a69292766d57ab36b4e072ef1f0a1248",
            'pro': "764e49dbc936d317dbdd897f4fdea489"  # api 用户
        }
    ...
    def set_environment(self, env):
        """Set the current environment (local, test, pro)"""
        if env in ['local', 'test', 'pro']:
            self.current_env = env
        else:
            raise ValueError("Invalid environment. Must be 'local', 'test', or 'pro'")

    @property
    def file_path(self):
        return self.paths[self.current_env]

    @property
    def minio_url(self):
        return self.minio_urls[self.current_env]

    @property
    def api_url(self):
        return self.api_urls[self.current_env]

    @property
    def minio_key(self):
        return self.minio_credentials[self.current_env][0]

    @property
    def minio_value(self):
        return self.minio_credentials[self.current_env][1]

    @property
    def login_password(self):
        return self.login_passwords[self.current_env]


# Usage example:
config = EnvironmentConfig()

# Set to desired environment (local, test, pro)
config.set_environment('pro')  # or 'local' or 'pro'

# Access configuration values
file_path = config.file_path
minio_url = config.minio_url
api_url = config.api_url
minio_key = config.minio_key
minio_value = config.minio_value
...
def update_mail_and_send(file_mail: dict, headers, link: str, password: str):
    ...
    调用接口，获取详细信息，供发送邮件使用
    """
    detail_mail_url = f"{api_url}/api/xiang-mail/mail/detail?id={mail_id}"
    ...
    调用接口，修改当前最新的文件信息
    """
    add_mail_url = f"{api_url}/api/xiang-mail/mail/submit"
    ...
    try:
        ...
        if mail_succ is True:
            ...
            send_mail_url = f"{api_url}/api/xiang-mail/mail/send-mail"
            try:
...
def send_dapi_server(file_mail_info):
    ...
    """
    get_token_url = (
        f"{api_url}/api/xiang-auth/oauth/token"
        "?tenant_id=000000"
        "&username=api"
    ...
    try:
        ...
        if access_token:
            ...
            策略： 3 天内，下载 1 次
            """
            send_dapi_exchange_url = f"{api_url}/api/xiang-exchange/exchange-log/add-share"
